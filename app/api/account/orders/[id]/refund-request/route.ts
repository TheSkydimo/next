...
export async function POST(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await requestRefundForUser(orderId, payload.userId);
    ...
    console.error("Request refund for account order error:", error);
    return NextResponse.json(
      { code: "INTERNAL_ERROR", message: "申请退款失败，请稍后重试" },
      { status: 500 },
    );
  }
}
